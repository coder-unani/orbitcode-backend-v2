...
export default function VideoDetailsCarousel({ video }: Props) {
  const theme = useTheme();

  const slides = video.thumbnail.map((img) => ({
    src: `https://storage.reviewniverse.net/${img.url}`,
  }));
      ...
      >
        {slides.map((slide) => (
          <Image
            ...
            sx={{ cursor: 'zoom-in', width: '100%', height: 'auto', objectFit: 'contain' }}
          />
        ))}
      </Carousel>
